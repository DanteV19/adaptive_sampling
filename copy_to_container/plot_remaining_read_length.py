...
try:
    ...
    combined_df['avg'] = combined_df[0] / combined_df[1]
    print("\nUpdated DataFrame:")
    print(combined_df)
...
avg_values = combined_df['avg'].values.reshape(-1, 2)  # Reshape into pairs for each group
...
# Plot the bars
rects1 = ax.bar(x - width / 2, avg_values[:, 0], width, label=bar_labels[0], color='blue')
rects2 = ax.bar(x + width / 2, avg_values[:, 1], width, label=bar_labels[1], color='green')

# Add labels, title, and legend
ax.set_xlabel('Duration per playback run', fontsize=14)
ax.set_ylabel('Average remaining length from unblocked reads', fontsize=14)
ax.set_title('Average remaining length from unblocked reads with\ntwo adaptive sampling methods per playback run',
             fontsize=14)
ax.set_xticks(x)
...
plt.ylim(top = float(max(combined_df['avg'].values)) * 1.20)
...
plt.savefig("remaining_bases.png")
